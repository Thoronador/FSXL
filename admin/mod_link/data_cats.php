...
$FSXL[title] = $FS_PHRASES[link_cats_title];
$FSXL[content] = '';

// Kategorie erstellen
if ($_POST[action] == 'newcat' && $_POST[name])
{
	$index = mysql_query("INSERT INTO `$FSXL[tableset]_link_cat` (`id`, `name`, `text`) 
							VALUES (NULL, '$_POST[name]', '$_POST[text]')");
...
if ($_POST[action] == 'editcat' && $_POST[name])
{
	...
	if($_POST[del])
	{
		mysql_query("UPDATE `$FSXL[tableset]_link` SET `cat` = $_POST[newcat] WHERE `cat` = $_POST[editid]");
		mysql_query("DELETE FROM `$FSXL[tableset]_link_cat` WHERE `id` = $_POST[editid]");
		$FSXL[content] = '
	...
	{
		$index = mysql_query("UPDATE `$FSXL[tableset]_link_cat` SET `name` = '$_POST[name]', `text` = '$_POST[text]'
...
elseif ($_GET[id])
{
	...
	$index = mysql_query("SELECT * FROM `$FSXL[tableset]_link_cat` WHERE `id` = $_GET[id]");
	$cat = mysql_fetch_assoc($index);

	$FSXL[content] = '
				<div>
				<form action="?mod=link&go=cats" method="post">
				...
				<table border="0" cellpadding="0" cellspacing="0" width="90%" style="margin:0px auto;">
					<tr>
						<td colspan="2"><span style="font-size:12pt;"><b>'.$FS_PHRASES[link_cats_editcat].'</b></span><hr></td>
					</tr>
					<tr>
					...
					<tr>
						<td valign="top"><b>'.$FS_PHRASES[link_cats_text].':</b></td>
...
else
{
	$FSXL[content] .= '
				...
				<table border="0" cellpadding="0" cellspacing="0" width="90%" style="margin:0px auto;">
					<tr>
						<td colspan="2"><span style="font-size:12pt;"><b>'.$FS_PHRASES[link_cats_newcat].'</b></span><hr></td>
					</tr>
					<tr>
						<td><b>'.$FS_PHRASES[link_cats_name].':</b></td>
						<td align="right">
				...
				<table border="0" cellpadding="2" cellspacing="1" width="90%" style="margin:0px auto;">
					<tr>
						<td colspan="2" style="padding:0px;"><span style="font-size:12pt;"><b>'.$FS_PHRASES[link_cats_editcat].'</b></span><hr></td>
					</tr>
					<tr>
						<td class="alt0"><b>'.$FS_PHRASES[link_cats_name].'</b></td>
						<td class="alt0" align="center"><b>'.$FS_PHRASES[link_cats_link].'</b></td>
					</tr>
	';
	...
	while ($cat = mysql_fetch_assoc($index))
	{
		...
		$FSXL[content] .= '
					<tr>
						<td class="alt'.($i%2==0?1:2).'"><a href="?mod=link&go=cats&id='.$cat[id].'">'.$cat[name].'</a></td>
						<td align="center" class="alt'.($i%2==0?1:2).'"><a href="../index.php?section=links&cat='.$cat[id].'" target="_blank"><img border="0" src="images/'.$FSXL[style].'_link.gif" alt=""></a></td>
					</tr>
		';
	}
